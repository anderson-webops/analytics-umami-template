...
test('POST fails closed when update permission changes during creation', async () => {
  parseRequestMock.mockResolvedValue({
    auth: { user: { id: 'user-1' } },
    body: { date: new Date(), allDay: true, note: 'deployment' },
    error: undefined,
  } as any);
  canUpdateWebsiteMock.mockResolvedValue(true);
  createAnnotationMock.mockRejectedValue(new Error('ENTITY_ACTOR_NOT_AUTHORIZED'));
