...
async function relationalQuery(
  ...
  await transaction.websiteEvent.create({
    data: {
      ...
      isBot,
      botName: truncateString(botName, 100),
      botCategory: truncateString(botCategory, 50),
      lcp,
      inp,
      cls,
      fcp,
...
async function clickhouseQuery({
  ...
  const message = {
    ...
    is_bot: isBot ? 1 : 0,
    bot_name: truncateString(botName, 100) || '',
    bot_category: truncateString(botCategory, 50) || '',
