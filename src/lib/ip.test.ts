...
describe('getIpAddress', () => {
  ...
  test('rejects malformed client addresses', () => {
  ...
  test('does not trust fallback headers when the configured edge header is absent', () => {
    vi.stubEnv('CLIENT_IP_HEADER', 'x-custom-ip');
    expect(getIpAddress(headers({ 'x-real-ip': '192.0.2.9' }))).toBeUndefined();
  });

  test('requires an explicitly trusted client IP header in production', () => {
...
describe('stripPort', () => {
  test('normalizes nullish and empty input to undefined', () => {
    expect(stripPort(undefined)).toBeUndefined();
    expect(stripPort(null)).toBeUndefined();
    expect(stripPort('')).toBeUndefined();
  });
  ...
  test('removes brackets and the port from a bracketed IPv6 address', () => {
